...
function ChangePasswordModal({
  ...
  onClose: () => void;
  onChangePassword: (current: string, newPw: string) => boolean;
}) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const ok = onChangePassword(currentPw, newPw);
    if (!ok) {
      ...
      >
        {success ? (
          <div className="flex flex-col items-center py-4">
            ...
            <p className="font-bold mt-3" style={{ color: "#059669" }}>비밀번호 변경 완료</p>
          </div>
        ) : (
          <>
            <div className="flex items-center gap-2 mb-5">
              <Key size={16} style={{ color: "#4F46E5" }} />
              <h2 className="font-bold text-sm" style={{ color: "#0F172A" }}>비밀번호 변경</h2>
            </div>
...
export function AdminPage() {
  const { isAuthenticated, login, logout, changePassword } = useAdminAuth();
      ...
      <div className="max-w-[1400px] mx-auto px-4 md:px-8 py-6 space-y-6">
        ...
        <div className="flex items-center justify-between pt-4 pb-2" style={{ borderTop: "1px solid #E2E8F0" }}>
          ...
          <span className="text-[11px]" style={{ color: "#CBD5E1" }}>
            상태 변경 및 서비스 추가는 이 브라우저의 localStorage에 저장됩니다
          </span>
        </div>
      </div>
