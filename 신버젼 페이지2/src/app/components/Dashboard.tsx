...
export function Dashboard() {
  ...
  // Build all services including custom ones
  const allServices: Service[] = useMemo(() => {
    const builtIn = servicesData.map((s) => ({
      id: s.id,
      name: s.name,
    ...
    <DashboardLayout searchQuery={searchQuery} onSearchChange={setSearchQuery}>
      <div className="max-w-[1600px] mx-auto space-y-6">
        ...
        {networkAvailable === false && (
          ...
          >
            <CloudOff size={16} style={{ color: "#EA580C" }} />
            <div className="flex-1 min-w-0">
              <p className="text-xs font-semibold" style={{ color: "#9A3412" }}>
                내부 네트워크 외부에서 접속 중
              </p>
              <p className="text-[11px] mt-0.5" style={{ color: "#C2410C" }}>
                서버 실시간 상태 확인은 사내 네트워크(203.242.139.254)에서만 가능합니다. 서비스 상태는 관리자 설정값을 표시합니다.
              </p>
            </div>
            <button
              onClick={checkAll}
              disabled={isChecking}
              className="flex items-center gap-1 px-3 py-1.5 rounded-lg text-[11px] font-semibold flex-shrink-0 transition-all hover:opacity-80"
              style={{ background: "#FFF", color: "#EA580C", border: "1px solid #FED7AA" }}
            >
              <RefreshCw size={10} style={{ animation: isChecking ? "spin 1s linear infinite" : "none" }} />
              재시도
            </button>
          </motion.div>
        )}
