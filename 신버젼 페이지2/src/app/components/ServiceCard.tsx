...
export function ServiceCard({ service, index, compact, isFavorite, onToggleFavorite, health }: ServiceCardProps) {
  ...
  const isProduction = typeof window !== "undefined" && window.location.hostname === "ec21rnc-agent.com";

  const handleClick = () => {
    if (isClickable) {
      const url = isProduction && path ? path : `http://203.242.139.254:${port}`;
        ...
        >
          ...
          {health && (
            <span
              ...
              style={{ background: healthMeta[health].color }}
            />
          )}
        </div>
