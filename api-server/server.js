...
async function generateNginxConf() {
  ...
  const blocks = withPath.map((s, i) => {
    ...
    // JS snippet: runs BEFORE the app loads
    // 1. Strip subpath prefix from URL so client-side routers (React Router, Vue Router, etc.) see "/"
    // 2. Patch fetch/XHR/pushState/replaceState etc. to ADD the prefix back for all outgoing requests/navigations
    const jsSnippet = `!function(){if(window.__pathRewritten)return;window.__pathRewritten=1;var b='${p}';function r(u){return typeof u==='string'&&'/'===u[0]&&0!==u.indexOf(b)?b+u:u}var hp=history.pushState;var hr=history.replaceState;var loc=location.pathname;if(loc.indexOf(b)===0){hr.call(history,null,'',(loc.slice(b.length)||'/')+location.search+location.hash)}history.pushState=function(s,t,u){return hp.call(this,s,t,r(u))};history.replaceState=function(s,t,u){return hr.call(this,s,t,r(u))};var f=window.fetch;window.fetch=function(u,o){return f.call(this,r(u),o)};var x=XMLHttpRequest.prototype.open;XMLHttpRequest.prototype.open=function(){arguments[1]=r(arguments[1]);return x.apply(this,arguments)};var E=window.EventSource;E&&(window.EventSource=function(u,o){return new E(r(u),o)});var wo=window.open;window.open=function(){arguments[0]=r(arguments[0]);return wo.apply(this,arguments)};if(navigator.sendBeacon){var sb=navigator.sendBeacon.bind(navigator);navigator.sendBeacon=function(u,d){return sb(r(u),d)}}var WS=window.WebSocket;WS&&(window.WebSocket=function(u,p2){return p2?new WS(r(u),p2):new WS(r(u))});var Wk=window.Worker;Wk&&(window.Worker=function(u,o){return new Wk(r(u),o)});if(window.jQuery){var aj=jQuery.ajax;jQuery.ajax=function(u,s2){if(typeof u==='string'){u=r(u)}else if(u&&u.url){u.url=r(u.url)}return aj.call(this,u,s2)}}if(window.axios){var ai=window.axios.interceptors;ai&&ai.request&&ai.request.use(function(c){if(c.url)c.url=r(c.url);return c})}}();`;
